...
export function BenchmarkView({
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm">
        Across this {benchmark.mission_count}-mission synthetic benchmark,
        AstraOS was selected in {pct(astra?.selection_rate ?? 0)} of simulated
        missions under the declared buyer utility model.
      </p>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-2">
        <div>
          <div className="h-[300px] border border-line bg-surface px-3 py-3">
            <p className="mb-2 text-xs text-muted">
              Simulated selection rate vs contribution per opportunity.
              Top-right is stronger buyer/merchant trade-off.
            </p>
                ...
                <Tooltip
                  content={({ payload }) => {
                    const point = payload?.[0]?.payload as
                      { name: string; x: number; y: number } | undefined;
        ...
        <div>
          ...
          >
            Full metrics, buyer segments and ablation →
          </button>
        </div>
      </div>
      <Drawer
        ...
        <div className="space-y-5">
              ...
              <tbody>
                {benchmark.strategy_metrics.map((row) => (
                  <tr key={row.strategy_name} className="border-b border-line">
                    ...
                    <td>
                      {row.avg_buyer_utility != null
                        ? formatUtilityShort(row.avg_buyer_utility)
                        : "—"}
                    </td>
                    <td>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Selection rate by scenario. AstraOS is adaptive, not a hardcoded
              winner.
            </p>
                      ...
                      {benchmark.strategies.map((name) => {
                        ...
                        return (
                          <td
                            key={name}
                            className={
                              rate >= 0.5 ? "font-medium" : "text-muted"
                            }
                          >
                            {row ? pct(rate) : "—"}
                          </td>
...
function AblationWaterfall({
  ...
  const first =
    semantic.contribution_per_opportunity_cents -
    def.contribution_per_opportunity_cents;
  const second =
    astra.contribution_per_opportunity_cents -
    semantic.contribution_per_opportunity_cents;
  return (
    <div>
      ...
      <ul className="mt-2 space-y-1 text-sm">
        <li>
          Default {formatAudCents(def.contribution_per_opportunity_cents)}
        </li>
        <li className="text-muted">
          → Semantic Only{" "}
          {formatAudCents(semantic.contribution_per_opportunity_cents)} (
